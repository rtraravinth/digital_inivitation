...
async def test_a_view_is_counted_and_attributed(client, auth_client, published):
    response = await client.post(
        f"/api/v1/public/p/{published['slug']}/views",
...
async def test_clicks_are_counted_per_section(client, auth_client, published):
    ...
    for _ in range(2):
        await client.post(
            f"/api/v1/public/p/{published['slug']}/clicks",
...
async def test_recording_against_an_unpublished_page_is_404(auth_client, client, portfolio):
    response = await client.post(
        f"/api/v1/public/p/{portfolio['slug']}/views",
        json={"referrer": "", "dedupeKey": "k"},
    )
...
async def test_reset_clears_my_events_only(
    ...
    await client.post(
        f"/api/v1/public/p/{published['slug']}/views", json={"referrer": "", "dedupeKey": "a"}
    )
    await client.post(
        f"/api/v1/public/p/{theirs['slug']}/views", json={"referrer": "", "dedupeKey": "b"}
    )
