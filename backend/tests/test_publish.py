...
async def test_an_unknown_slug_is_404(client):
    response = await client.get("/api/v1/public/p/nobody-here")
    assert response.status_code == 404
...
async def test_a_published_page_is_readable_without_a_token(client, published):
    ...
    response = await client.get(f"/api/v1/public/p/{published['slug']}")
...
async def test_an_unpublished_page_stops_being_readable(auth_client, client, published):
    ...
    assert (await client.get(f"/api/v1/public/p/{published['slug']}")).status_code == 404
...
async def test_hidden_sections_never_reach_the_published_payload(
    ...
    await auth_client.patch(
        f"/api/v1/portfolios/{published['id']}/sections/{section_id}", json={"hidden": True}
    )
...
async def test_show_contact_off_removes_the_header_links_from_the_body(
    ...
    await auth_client.patch(
        f"/api/v1/portfolios/{published['id']}/header",
        json={"links": [{"id": "l1", "label": "Email", "url": "rohan@northwell.in"}]},
    )
    assert (await client.get(f"/api/v1/public/p/{published['slug']}")).json()["header"]["links"]
...
async def test_badge_off_is_reported_to_the_page(auth_client, client, published):
    ...
    assert (await client.get(f"/api/v1/public/p/{published['slug']}")).json()["badge"] is False
...
async def test_a_published_asset_is_public_and_cacheable(auth_client, client, published):
    ...
    await auth_client.patch(
        f"/api/v1/portfolios/{published['id']}/header", json={"portraitAssetId": asset["id"]}
    )
...
async def test_the_published_payload_uses_public_asset_urls(auth_client, client, published):
    asset = (
        await auth_client.post(
            "/api/v1/assets",
            files={"file": ("h.png", png_bytes(), "image/png")},
    ...
    section_id = published["sections"][0]["id"]
    await auth_client.patch(
        f"/api/v1/portfolios/{published['id']}/sections/{section_id}",
...
async def test_an_asset_on_a_hidden_section_is_not_public(auth_client, client, published):
    asset = (
        await auth_client.post(
            "/api/v1/assets",
